...
def require_published_artifact(path: Path, source_hash: str | None = None) -> dict[str, Any]:
    ...
    payload = json.loads((ROOT / "reproducibility" / "manifests" / "hashes" / "artifact_provenance.json").read_text(encoding="utf-8"))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=ROOT / "results" / ("local_vscode_comparison_" + datetime.now().strftime("%Y%m%d_%H%M%S")))
