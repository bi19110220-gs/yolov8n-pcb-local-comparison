...
The beginner interface is [one level above](../README.md). This folder retains the exact configurations, authorities, provenance, notices, tests, and historical context behind the reported comparison.
...
Exported runtimes include `train_local.py`, `prepare_dataset.ps1`, their internal code, requirements, and the published dataset checksum. The full-repository verification wrapper remains in this checkout because a staging export does not contain the complete Git repository, report, or test suite. Run the verification commands above against this checked-in package, not an evidence-only staging export.
