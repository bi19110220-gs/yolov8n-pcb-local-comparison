...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="pcb fresh clone ") as temporary:
        ...
        if failures:
            ...
            return 1
        for command in (
            [sys.executable, "-m", "tools.run_local_vscode_comparison", "--help"],
            [sys.executable, "-c", "from tools.run_local_vscode_comparison import verify_packaged_inputs; verify_packaged_inputs()"],
            [sys.executable, "scripts/verify/verify_package.py", "--root", "."],
        ):
