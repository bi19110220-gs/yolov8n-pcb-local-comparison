...
RUNTIME_ITEMS = tuple((name, name) for name in (
    ...
    "tools/prepare_dataset.ps1", "tools/verify_package.ps1", "scripts/package/extract_dataset.py",
))
...
def export_portable_authorities(source_root: Path, run_root: Path, destination_root: Path) -> list[dict[str, Any]]:
    ...
    destination = destination_root / "manifests/dataset/authority"
...
def package(source_root: Path, run_root: Path, destination_root: Path) -> dict[str, Any]:
    ...
    atomic_json(
        destination_root / "manifests" / "hashes" / "artifact_provenance.json",
        payload,
    )
    return payload
